...
class ImageServer {
    ...
    _resolveRotate(rotate) {
        ...
        rotateNumber = rotateNumber <= 90 ? 1 : Math.round(rotateNumber / 90)
    ...
    async stream(response, sourceImageStat, imageHandleOptions) {
        ...
        if (imageHandleOptions.size) {
            transformer = transformer.resize(imageHandleOptions.size.width, imageHandleOptions.size.height)
        ...
        let ext = path.extname(handledImagePath)

        response.setHeader('Image-Name', path.basename(handledImagePath, ext))

        response.setHeader('Content-Type', MIMES[ext])

        transformer.clone().pipe(fileStream.createWriteStream(handledImagePath).on('error', err => console.error(err)))
